	...
	if(check_bitrix_sessid())
	{
		if(empty($arParams["REQUIRED_FIELDS"]) || !in_array("NONE", $arParams["REQUIRED_FIELDS"]))
		{
			...
			if((empty($arParams["REQUIRED_FIELDS"]) || in_array("EMAIL", $arParams["REQUIRED_FIELDS"])) && mb_strlen($_POST["user_email"]) <= 1)
				...
				if((empty($arParams["REQUIRED_FIELDS"]) || in_array("OBJECT", $arParams["REQUIRED_FIELDS"])) && mb_strlen($_POST["user_object"]) <= 1)
				$arResult["ERROR_MESSAGE"][] = GetMessage("MF_REQ_OBJECT");	
		...
		$arResult["AUTHOR_OBJECT"] = htmlspecialcharsbx($_POST["user_object"]);
	}
	else
